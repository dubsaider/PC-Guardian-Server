...
class KafkaConfig:
    ...
    def __init__(self, config_file: Optional[str] = None):
        ...
        self.consumer_group = os.getenv('KAFKA_CONSUMER_GROUP', 'pc-guardian-server')
        
        # Загрузка из файла конфигурации, если указан
    ...
    def _load_from_file(self, config_file: str):
        ...
        try:
            with open(config_file, 'r', encoding='utf-8') as f:
                ...
                self.security_protocol = config.get('security_protocol', self.security_protocol)
                ...
                self.sasl_mechanism = config.get('sasl_mechanism', self.sasl_mechanism)
                self.sasl_username = config.get('sasl_username', self.sasl_username)
                self.sasl_password = config.get('sasl_password', self.sasl_password)
                self.consumer_group = config.get('consumer_group', self.consumer_group)
